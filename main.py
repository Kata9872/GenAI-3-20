...
def main():
    ...
    parser = argparse.ArgumentParser(description="Генератор рекламных креативов")
    parser.add_argument("--product", type=str, required=True, help="Название продукта")
    parser.add_argument("--style", type=str, required=True, help="Стиль слогана")
    args = parser.parse_args()

    prompt = f"""Ты — профессиональный копирайтер. Создай 3 рекламных слогана для продукта «{args.product}» в стиле «{args.style}». 
    Также напиши краткое описание рекламной акции (1–2 предложения). 
    Не добавляй пояснений, нумерацию начинай с 1., оформи ответ чётко и структурированно."""
    ...
    result = generate_text(generator, messages, max_new_tokens, do_sample, temperature, top_p, repetition_penalty)
    # Вывод сгенерированного текста без промпта

    print("=== Сгенерированные рекламные креактивы ===")
    print(result)

    output_file = "output.md"
    try:
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"## Рекламные креативы для: {args.product}\n\n")
            f.write(f"### Стиль: {args.style}\n\n")
            f.write(result)
        print(f"\nКреативы успешно сохранены в файл: {output_file}")
